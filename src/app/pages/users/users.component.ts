...
export class UsersComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  public columns = [
    ...
    {
      field: "created_at",
      header: "FormUser.COLUMNS.6.header",
      isText: true,
      isCenter:true,
    },
    {
      field: "updated_at",
      header: "FormUser.COLUMNS.7.header",
      isText: true,
      isCenter:true,
    },
  ];
  ...
  public async handleUpdateStatus(data): Promise<void> {
    const swalOptions = this.getSwalUpdateStatusOptions(`${data.nom} ${data.prenoms}`);
    const result = await Swal.fire(swalOptions);
    if (result.isConfirmed) {
  ...
  private mapUserData(): any[] {
    return this.listUser.map((element: any) => ({
      ...
      [this.translate.instant("FormUser.COLUMNS.3.header")]: element?.telephone,
      [this.translate.instant("FormUser.COLUMNS.4.header")]: element?.message,
      [this.translate.instant("FormUser.COLUMNS.5.header")]: element?.status,
      [this.translate.instant("FormUser.COLUMNS.6.header")]: element?.createdat,
      [this.translate.instant("FormUser.COLUMNS.7.header")]: element?.updatedat,
    }));
  }
